# -*-python-*-
# $Id: xmlgenerator.py,v 1.14 2007/12/26 02:55:40 asc Exp $
...
__date__       = "$Date: 2007/12/26 02:55:40 $"
...
class xmlgenerator (base) :
    ...
    def write (self, fh=stdout, format="edfg11") :
    ...
    def write_edfg11 (self) :
        ...
        self.xml.startElementNS((NS_MAP['e'], 'eatdrinkfeelgood'), None, lang)
        self.xml.startElementNS((NS_MAP['e'], 'recipe'), None, {})
        
        if self.isset_notempty(self.data, 'title') :
            self.write_titles(self.data['title'])

        if self.isset_notempty(self.data, 'source') :
            self.write_source(self.data['source'])

        if self.isset_notempty(self.data, 'yield') :
            self.write_yield(self.data['yield'])

        if self.isset_notempty(self.data, 'subject') :
            self.write_categories(self.data['subject'])
        ...
        self.xml.endElementNS((NS_MAP['e'], 'recipe'), None)        
        self.xml.endElementNS((NS_MAP['e'], 'eatdrinkfeelgood'), None)        
    ...
    def write_categories (self, subjects) :
        ...
        for tag in subjects :
            attrs = AttributesNSImpl({(NS_MAP['e'], 'type'):'name',
    ...
    def write_ingredient (self, ing) :
        ...
        if ing.has_key('content') :
            ...
            for ln in ing['content'].split("\n\n") :
                self.xml.startElementNS((NS_MAP['e'], 'ing'), None, {})
                self.write_item(ln.strip())                
